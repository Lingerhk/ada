...
func (n *localConn) writeString(framer Framer, formatter Formatter, p Priority, hostname, program, tag, msg string) error {
	if framer == nil {
	...
	_, err := n.conn.Write([]byte(framer(formatter(p, hostname, program, tag, msg))))
	return err
}
