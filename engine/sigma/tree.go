...
func (t Tree) Eval(e Event) (*Result, bool) {
	...
	if match {
		// extract fields for alerts.
		fieldMap := make(map[string]string)
		for _, field := range t.Rule.Fields {
			if val, ok := e.Select(field); ok {
				...
				case []interface{}:
					...
					fieldMap[field] = strings.Join(ret, ",")
				default:
					//
				}
			}
		...
		// extract fields for unique id.
		var uniqueFieldStr, uniqueId string
		for _, field := range t.Rule.UniqueFields {
			if val, ok := e.Select(field); ok {
				switch v := val.(type) {
				case int:
		...
		if uniqueFieldStr != "" {
			// 计算md5
			hasher := md5.New()
			hasher.Write([]byte(uniqueFieldStr))
			uniqueId = hex.EncodeToString(hasher.Sum(nil))
		}
		...
		return &Result{
			...
			Status:      t.Rule.Status,
			Level:       t.Rule.Level,
			Fields:      fieldMap,
			UniqueId:    uniqueId,
			Timestamp:   time.Now().UnixNano() / 1e6, // 单位是毫秒
		}, true
	}
	return nil, false
}
