		...
		go func(workerID int) {
			...
			for {
				...
				case <-ticker.C:
					...
					err = w.backend.SetResult(taskMessage.TaskID, resultMsg)
					if err != nil {
...
func (w *CeleryWorker) RunTask(message *TaskMessage) (*ResultMessage, error) {
	...
	if message.Expires != nil && message.Expires.UTC().Before(time.Now().UTC()) {
		return nil, fmt.Errorf("task %s is expired on %s", message.TaskID, message.Expires)
	}

	// check for malformed task message - args cannot be nil
	if message.Args == nil {
		return nil, fmt.Errorf("task %s is malformed - args cannot be nil", message.TaskID)
	}
